## MVC
Para acessar a aplicação com o thymeleaf, basta acessar http://localhost:8080, te redicionara para a tela de login caso ainda não tenha autenticado, ou para a pagina central de livros se autenticado.

## Security
Eu optei por uma abordagem mais simples da aplicação do Spring Security, utilizando usuários em memória. Existem dois usuários:
```
Username: cliente
Password: cliente
Permissões somente para visualizar os livros
```
```
Username: atendente
Password: atendente
Permissões para visualizar, editar totalmente ou parcialmente e deletar livros e usuários, além de fazer empréstimos ou devolver.
```

Além das páginas de lista e formulário de livros, usuários e empréstimos. Temos a página customizada de login e acesso negado.
A página de acesso negado pode ser acessada logando com o usuário cliente e tentando acessar a uri /usuarios.
